...
export function QuotationExport({ quotation, organization, onClose }: { quotation: ExportQuotation; organization: ExportOrganization | null; onClose: () => void }) {
  ...
  async function handleDownloadPdf() {
    setDownloading(true);
    try {
      const effectiveStatus = await markAsSent(quotation.id);
      const blob = await pdf(<QuotationPdfDocument quotation={{ ...quotation, status: effectiveStatus }} organization={organization} />).toBlob();
