...
type Cotizacion = {
  ...
  cliente?: RelatedOne<{ nombre: string }>;
  cotizacionxservicio?: {
    cantidad: number;
    precio_unitario: number;
    descuento?: number | null;
    servicio?: RelatedOne<{ nombre: string }>;
  }[] | null;
};
...
export default async function QuotationsPage({ searchParams }: Props) {
  ...
  const [quotationsResult, customersResult, servicesResult] = await Promise.all([
    supabase.from("cotizacion").select("id_cotizacion, estado, total_cotizado, subtotal_general, impuesto, descuento, fecha, observacion, cliente(nombre), cotizacionxservicio(cantidad, precio_unitario, descuento, servicio(nombre))").order("fecha", { ascending: false }),
